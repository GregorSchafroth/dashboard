...
async function fetchAllHistoricalTranscripts() {
  try {
    ...
    for (const project of projects) {
      ...
      if (!response.ok) {
        console.error(`Failed to fetch transcripts for project ${project.name}:`, response.status)
        continue
      }

      const transcripts = (await response.json()) as VoiceflowTranscript[]
      console.log(`Found ${transcripts.length} transcripts for project ${project.name}`)
      ...
      for (let i = 0; i < transcripts.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (transcript) => {
          try {
            ...
            processedCount++
            console.log(`Processed ${processedCount}/${transcripts.length} transcripts for ${project.name}`)
          } catch (error) {
...
async function processTranscript(project: ProjectWithAuth, transcript: VoiceflowTranscript) {
...
function calculateTranscriptMetrics(turns: VoiceflowTurn[]): TranscriptMetrics {
  ...
  const firstResponse = timestamps.length > 0 
    ? new Date(Math.min(...timestamps.map(date => date.getTime())))
    : null
  const lastResponse = timestamps.length > 0 
    ? new Date(Math.max(...timestamps.map(date => date.getTime())))
    : null

  const duration = firstResponse && lastResponse
    ? Math.round((lastResponse.getTime() - firstResponse.getTime()) / 1000)
    : null

  const lastTurn = turns[turns.length - 1]
  const isComplete = lastTurn?.type === 'choice' || 
    (lastTurn?.type === 'text' && !turns.some(t => t.type === 'request' && new Date(t.startTime) > new Date(lastTurn.startTime)))
